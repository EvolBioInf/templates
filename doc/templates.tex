...
\textheight=23cm
\begin{document}
\title{\texttt{templates} \input{version}: <Description>}
...
scratch. The package \ty{templates} consists of programs for
...
\end{verbatim}
The \ty{-a} option for \emph{author} is mandatory.
\item Look at the new file
\begin{verbatim}
cat foo.c
\end{verbatim}
\item If you repeat the last command, \ty{makeCfile} refuses to
...
\begin{itemize}
  \item The directory \texttt{common} contains the files \texttt{eprintf.c},
    \texttt{eprintf.h}, and \ty{Makefile}. The files
  ...
  \item The file \ty{Makefile} is used for making the project.
  \item The file \ty{README.md} contains a brief description of the
